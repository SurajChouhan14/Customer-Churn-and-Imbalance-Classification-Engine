...
Verifies Data Ingestion, Preprocessing, Out-of-Sample ROC-AUC / PR-AUC Metrics, and 5-Fold CV Threshold Optimization.
"""
...
class TestCustomerChurnEngine(unittest.TestCase):
    ...
    def test_2_data_preprocessing(self):
    ...
    def test_3_model_auc_performance(self):
        """Verify model achieves out-of-sample ROC-AUC > 0.80 and PR-AUC > 0.60 on held-out test data."""
    ...
    def test_4_bayesian_cost_optimization(self):
        """Verify optimal cost threshold is tuned lower than default 0.50 cutoff."""
        _, metrics = self.engine.train_and_evaluate(self.df, test_size=0.20)
        self.assertLess(metrics["optimal_threshold"], 0.45)
        self.assertGreater(metrics["optimal_threshold"], 0.15)
        self.assertGreater(metrics["cost_savings_pct"], 0.0)
